...
Second, we want to investigate several methodical questions that are inherent to the current practice of choice experiments:
...
-   **Hypothesis T2**\label{hyp-T2}: Provision of an interactive map at the beginning of the travel cost study part and subsequent display of selected location on the map in trip-number and trip-information elicitation questions compared to provision of a map at the end of the travel cost study part:
...
-	 In this interdisciplinary article, we analyse the interplay between psychological variables and the willingness to pay for natural goods and services. We estimate a structural equation model explaining the relationships between NR using a scale developed by @Nisbet2013, WTP and health/wellbeing. Our goal is to identify society characteristics that are relevant for the success of, and willingness to invest in nature conservation efforts.
...
-   Biodiversity is conceptualized in the form of biotope value points. This article delves into the markets for biodiversity, examining how transaction prices relate to subjective willingness to pay and welfare considerations. The authors therefore combine price estimates per biotope value point obtained from an expert survey of the ValuGaps research project with estimates from the survey described hereafter. The goal of the research is to translate biotope valuation methods into the standardized framework of Germany's Federal Compensation Ordinance in order to make an intent at assessing the overall value of German biotopes and habitats at market prices. The findings inform environmental economic accounting, policy scenario development, project planning, life cycle assessments, and corporate natural capital assessments. The paper also evaluates the methodology's potential for application beyond Germany, suggesting its broader relevance and utility in biodiversity valuation globally.

# Part B: Experimental Design {.unnumbered}

## Data and Experimental Setup \label{data-exp-setup}

The sample is a random sample of the adult population in Germany with representative quotas for age and gender, and additional soft quotas for income and region. The main quotas are replicated within the split samples. We will start the data collection in Germany with a sample size of N = 15,000. The sample comes from different panels from market research institutes in Germany.

We used simulations to determine the minimum required sample size to derive a power of 80% for a simple discrete choice conditional logit model with a specification assumed from earlier studies [@Sagebiel2017]. The sample size was 3000. As the survey was financed by a research project, we tried to maximize the sample size for the given project budget. We ended up with a total sample of N = 15,000.
...
**Soft launch**\
For the final data collection we started the survey with a soft launch with a target sample size of N = 500 to test whether all changes made after the two rounds of pretests have been implemented correctly. In this process we discovered a coding mistake in our R code used to calculate the status quo endowments of the respondents. Specifically, we had a mistake in the way how we calculated the amount of protected areas which resulted in lower than actual values for the endowment with protected areas, particular in regions with many small patches of protected areas. We changed the R code accordingly for the rest of the data collection.
...
This follow-up page consists of a series of matrix questions to be answered on a 5-point Likert scale. The questions are designed to elicit information about the respondents' general attitudes and considerations regarding the consequentiality of the survey. Respondents are then asked for their opinion on the distribution of costs between households. At the end of this part of the survey, we ask the respondents in a non-obligatory open question for any general comments on their response behavior in the previous sets of choices. The responses we collect at this point can complement the interpretation of the DCE data and potentially form the basis of independent qualitative research.
...
In the fifth part of the study, the participants are divided into two groups, which then receive different questions. Subsequently, one-third of the participants engage in an experiment examining distributional decisions between financial and environmental budget appropriation. The remaining two-thirds of participants are first required to complete the six-item questions of the nature-relatedness scale [@Nisbet2013]. The Nature Relatedness scale (NR6) is a 6-item self-report measure, allowing assessment of individual attitudes towards nature in the form of a“self” and “experience” scale. It is a short version of the original 21-item Nature Relatedness Scale. They are then asked to indicate their willingness to pay for a general programme that seeks to fulfil the EU biodiversity goals, using a contingent valuation question. At the conclusion of this section, participants are presented with questions regarding travel costs associated with their recreational visits to local environmental sites.  Between the pre-tests and the soft launch we extended the contents of the travel cost part and implemented four mutually independent treatments. The format of the questions in the travel cost study part will depend on the treatment group to which the respondent is assigned. The questions will inquire about the number of visits to local environmental sites and whether the interactive map is presented at the beginning and displayed on the following screens, or only at the end of the section. The four treatments are as follows: 1, 2, 3 and 4 (see the \textbf{\hyperref[travel-cost]{travel cost}} subsection for details). Approximately 50% of respondents will be randomly assigned to each of the two groups within each treatment. This results in approximately 6.25% of respondents being exposed to each of the 16 possible combinations of the survey. \newline
